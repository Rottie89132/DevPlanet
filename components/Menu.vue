<template>
    <div id="navbar" class="fixed top-0 z-10 w-1 h-full">
        <Transition name="navbar">
            <div v-if="NavShow">
                <button tabindex="0" @keydown.enter="ShowNavbar" @click="ShowNavbar" :disabled="busy" ref="navbar" class="absolute w-screen h-full bg-black cursor-default backdrop-blur-sm bg-opacity-60"></button>
                <Transition name="topics">
                    <div v-if="TopShow" ref="topics" class=" fixed border-r-2 dark:border-[#111111] border-[#efefef] dark:bg-[#090909] bg-[#f6f6f6] dark:text-[#ccc] text-[#262525] border-opacity-40 justify-center p-4  h-full">
                        ...
                        <NuxtLink to="/" class="btn-focus outline-none dark:hover:text-white hover:text-[#0a0a0a] bg-[#dadada] hover:bg-[#d0d0d0] dark:bg-[#100F0F] dark:hover:bg-[#1e1c1c] transition-all delay-100 mb-5 flex justify-center p-2 items-center rounded-lg">
                                <icon size="1.6rem" name="ic:baseline-home"></icon>
                        </NuxtLink>
                        <NuxtLink :to="redirectServer" class="btn-focus outline-none dark:hover:text-white hover:text-[#0a0a0a] bg-[#dadada] hover:bg-[#d0d0d0] dark:bg-[#100F0F] dark:hover:bg-[#1e1c1c] transition-all delay-100 mb-5 flex justify-center p-2 items-center rounded-lg">
                                <icon size="1.6rem" name="ic:baseline-apartment"></icon>
                        </NuxtLink>
                        <NuxtLink :to="redirect" class="btn-focus outline-none dark:hover:text-white hover:text-[#0a0a0a] bg-[#dadada] hover:bg-[#d0d0d0] dark:bg-[#100F0F] dark:hover:bg-[#1e1c1c] transition-all delay-100 mb-5 flex justify-center p-2 items-center rounded-lg">
                                <icon size="1.6rem" name="ic:baseline-all-inbox"></icon>
                        </NuxtLink>
                        <hr class=" my-5 border dark:border-[#100F0F] border-[#dadada]">
                        <div class="absolute bottom-3">
                            <hr class=" my-5 border dark:border-[#100F0F] border-[#dadada]">
                            <button class="btn-focus outline-none dark:hover:text-white hover:text-[#0a0a0a] bg-[#dadada] hover:bg-[#d0d0d0] dark:bg-[#100F0F] dark:hover:bg-[#1e1c1c] transition-all delay-100 mb-5 flex justify-center p-2 items-center rounded-lg" @click="logout">
    ...
    <button class="btn-focus flex items-center justify-between p-3 mx-3 mt-2 rounded-lg dark:bg-[#141414] bg-[#EBEBEB]" aria-label="menu" @click="ShowNavbar" :disabled="busy">
